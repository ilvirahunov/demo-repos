...
Before proceeding, please make sure you've read the other articles from this section, especially:
 ...
 - [Understanding the Handlebars (`.hbs`) templates](docs/5-development/04-understanding-hbs-templates.md)
...
See [Scoping](docs/2-advanced/03-scoping.md) for more on using a suffix with tag names.
...
Even if a suffix for tag names is configured (as described in [Scoping](docs/2-advanced/03-scoping.md)), the attribute with
...
The `individualSlots` slot metadata configuration setting (see [Understanding UI5 Web Components Metadata](docs/5-development/03-understanding-components-metadata.md)) allows you to have a separate physical slot for each child belonging to a certain slot.
...
For more information on individual slots and how to render them in the `.hbs` template click [here](docs/5-development/04-understanding-hbs-templates.md#slots_individual).
...
For more on the `.hbs` template syntax and event listeners click [here](docs/5-development/04-understanding-hbs-templates.md#syntax_at).
...
However, if you set the [noConflict](docs/2-advanced/01-configuration.md#no_conflict) configuration setting to `true`, only the **prefixed** event will be dispatched.
...
Finally, `invalidateOnChildChange` allows for more fine-granular rules when exactly children can invalidate their parents - see [Understanding UI5 Web Components Metadata](docs/5-development/03-understanding-components-metadata.md).
...
This section expands on the UI5 Web Components class structure, so if you haven't, please check [Developing Custom UI5 Web Components](docs/5-development/02-custom-UI5-Web-Components.md) first.
